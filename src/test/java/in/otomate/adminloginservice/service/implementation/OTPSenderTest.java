...
public class OTPSenderTest {
	...
	void testSendMailOtp() {
		OTPRequest otpRequest = OTPRequest.builder().fname("Dhamodaran").lname("N").contact("dhamodaran@outlook.in")
				.build();
		Boolean sent = service.sendOtp(otpRequest);
		assertEquals(true, sent);
	}
 
	void testSendMobileOtp() {
		OTPRequest otpRequest = OTPRequest.builder().fname("Dhamodaran").lname("N").contact("+919655606681")
				.build();
		Boolean sent = service.sendOtp(otpRequest);
		assertEquals(true, sent);
	}
	...
	void testVerifyMail() {
		...
		VerifyContact contact=VerifyContact.builder()
				.contact("dhamodaran@outlook.in")
				.otp("089432")
				.build();
		assertEquals(true, service.verify(contact));
	}
    @Test
    void testVerifyMobile() {

		VerifyContact contact=VerifyContact.builder()
				.contact("+919655606681")
				.otp("083451")
				.build();
		assertEquals(true, service.verify(contact));
    }
